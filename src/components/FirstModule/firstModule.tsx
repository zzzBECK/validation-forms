...
const formSchema = z.object({
    item1: z.array(z.string()).nonempty({ message: "Select at least one option" }),
    item2: z.array(z.string()).nonempty({ message: "Select at least one option" }),
    item3: z.array(z.string()).nonempty({ message: "Select at least one option" }),
    item4: z.array(z.string()).nonempty({ message: "Select at least one option" }),
    item5: z.array(z.string()).nonempty({ message: "Select at least one option" }),
    item6: z.array(z.string()).nonempty({ message: "Select at least one option" }),
    item7: z.array(z.string()).nonempty({ message: "Select at least one option" }),
});
...
const calculateScore = (itemName: keyof FormData, selectedOptions: string[]) => {
    switch (itemName) {
        case 'item1':
        ...
        case 'item3':
            return selectedOptions.length > 0 ? 1 : 0;
        case 'item4':
            if (selectedOptions.includes("4.1- Processo de alfabetização (1° e 2° anos do Ensino Fundamental)") &&
                selectedOptions.includes("4.2 - Recomposição das aprendizagens (3°, 4° e 5° anos do Ensino Fundamental)")) return 1;
            if (selectedOptions.includes("4.1- Processo de alfabetização (1° e 2° anos do Ensino Fundamental)")) return 0.75;
        ...
        case 'item6':
            ...
            if (selectedOptions.includes("6.4 – Contraturno (para além da carga horária)")) return 0.5;
            if (selectedOptions.includes("6.2- Dentro de 1/3 da carga horária (coordenações, hora de atividades, planejamentos e reuniões)")) return 0.75;
            if (selectedOptions.includes("6.1- Dentro da carga horária de trabalho")) return 1;
            return 0;
        case 'item7':
                                                ...
                                                {items[itemName as keyof FormData].map(value => (
                                                    <label key={value} className="flex items-center space-x-2">
                                                        <Checkbox
                                                            ...
                                                            onCheckedChange={handleCheckboxChange(field, itemName as keyof FormData, value)}
                                                        />
                                                        <span>{value}</span>
                                                    </label>
                                                ))}
